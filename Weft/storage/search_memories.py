...
from Weft.storage.models import Memory, MemoryType, Chunk, Embedding, Message
...
def search_memories(query: str):
    ...
    try:
        ...
        try:
            ...
            results = db.execute(
                select(Embedding.message_id, Embedding.embedding_vector.l2_distance(query_embedding).label('dist'))
                .order_by('dist')
                .limit(3)
            ).all()
            ...
            if results:
                for row in results:
                    ...
                    if msg:
                        print(f" - [Message {msg.id}] (Dist: {row.dist:.4f}): {msg.content[:100]}...")
            else:
                print(" - No semantic chunks found. (Have embeddings been created?)")

        except Exception as vec_err:
            print(f" - [Warning] Semantic search unavailable or failed: {vec_err}")
