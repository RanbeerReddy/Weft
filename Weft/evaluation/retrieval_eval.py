...
def format_report(summary: EvaluationSummary, verbose: bool = False) -> str:
    ...
    if summary.passed_queries:
        best_by_mrr = sorted(summary.passed_queries, key=lambda m: m.mrr, reverse=True)[:3]
        lines.append("BEST PERFORMING QUERIES (by MRR):")
        for m in best_by_mrr:
            lines.append(f"  • {m.query[:50]}")
            lines.append(f"    MRR: {m.mrr:.3f}, Hit@1: {m.hit_at_1}, Recall: {m.keyword_recall:.2f}")
        lines.append("")
    ...
    if summary.failed_queries:
        ...
        for m in worst:
            ...
            if m.retrieved_chunks:
                ...
                lines.append(f"    Top result (dist={top_chunk.distance:.3f}): {top_chunk.chunk_text[:60]}...")
        lines.append("")
    ...
    if verbose:
        ...
        for i, m in enumerate(summary.query_metrics, start=1):
            ...
            lines.append(f"    Hit@1/3/5/10: {m.hit_at_1}/{m.hit_at_3}/{m.hit_at_5}/{m.hit_at_10}")
            lines.append(f"    Recall: {m.keyword_recall:.2f}, Precision: {m.keyword_precision:.2f}, MRR: {m.mrr:.3f}")
            
            if m.retrieved_chunks:
                lines.append(f"    Top 3 retrieved chunks:")
                for chunk in m.retrieved_chunks[:3]:
                    lines.append(f"      [{chunk.rank}] dist={chunk.distance:.4f}: {chunk.chunk_text[:70]}...")
            lines.append("")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Evaluate Weft retrieval system'
    )
    parser.add_argument(
        '--queries',
        type=str,
