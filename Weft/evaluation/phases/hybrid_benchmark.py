...
from Weft.core.retrieval import VectorRetriever, LexicalRetriever, HybridRetriever
...
def convert_to_memory_result(results: List[RetrievalResult]) -> List[MemoryRetrievalResult]:
    memory_results = []
    for r in results:
...
def evaluate_strategy(queries: List[Dict[str, Any]], strategy: str, alpha: float = 0.5) -> MemoryEvaluationSummary:
    query_metrics = []
    ...
    for i, query_dict in enumerate(queries, start=1):
        ...
        if strategy == "semantic":
            ...
            raw_results = retriever.retrieve(query_text, k=50, fusion_strategy="linear", alpha=alpha)
    ...
    print(f"    -> Done in {elapsed:.2f}s | Hit@1: {summary.memory_hit_at_1_rate:.2%} | MRR: {summary.avg_phrase_mrr:.4f} | Latency/q: {(elapsed/len(query_metrics))*1000:.1f}ms")
    ...
    for q_text, sem_m in semantic_queries.items():
        ...
        if sem_rank != hyb_rank:
            if hyb_rank < sem_rank:
                ...
                print(f"  Q: {q_text}")
                print(f"  Rank: Semantic={sem_rank} -> Hybrid={hyb_rank} (Lexical={lex_rank})")
