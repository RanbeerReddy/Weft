...
def evaluate_system(queries: List[Dict[str, Any]], 
                    search_func, 
                    name: str) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
...
def run_benchmark():
    ...
    baseline_agg, baseline_results = evaluate_system(
        queries, 
        lambda q: retriever.retrieve(q, k=10), 
        "Baseline (Vector Top-10)"
    )
    ...
    for b_res, r_res in zip(baseline_results, reranker_results):
        ...
        if b_hit_rank != r_hit_rank:
            ...
            if r_hit_rank:
                ...
                print(f"  Explanation: The reranker successfully lifted this chunk from outside the baseline top-10 or from a lower rank by recognizing semantic interaction between query and chunk.")
