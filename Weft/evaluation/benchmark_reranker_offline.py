...
def evaluate_system(dataset: List[Dict[str, Any]], 
                    ...
                    name: str = "Baseline") -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
    ...
    # Aggregate
    agg = {
        "hit_at_1": sum(1 for r in results if r["metrics"].hit_at_1) / len(results) if results else 0,
        "hit_at_3": sum(1 for r in results if r["metrics"].hit_at_3) / len(results) if results else 0,
        "hit_at_5": sum(1 for r in results if r["metrics"].hit_at_5) / len(results) if results else 0,
        "hit_at_10": sum(1 for r in results if r["metrics"].hit_at_10) / len(results) if results else 0,
...
def run_benchmark():
    ...
    reranker_agg, reranker_results = evaluate_system(
        dataset, 
        reranker=reranker, 
        name="Pipeline (Vector Top-100 -> Reranker Top-10)"
    )
    ...
    for b_res, r_res in zip(baseline_results, reranker_results):
        b_hit_rank = next((c.rank for c in b_res["retrieved"] if MetricsCalculator.extract_keywords(c.chunk_text) & set(x.lower() for x in b_res["expected"])), None)
        r_hit_rank = next((c.rank for c in r_res["retrieved"] if MetricsCalculator.extract_keywords(c.chunk_text) & set(x.lower() for x in r_res["expected"])), None)
        ...
        if b_hit_rank != r_hit_rank:
            ...
            if r_hit_rank:
                ...
                print(f"  Reranker Score (saved as distance): {-correct_chunk.distance:.4f}")
                print(f"  Explanation: The reranker successfully identified semantic interaction between query and chunk.")
