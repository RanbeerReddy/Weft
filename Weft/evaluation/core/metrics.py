...
class MetricsCalculator:
    ...
    def evaluate_query(
        ...
        # Compute metrics
        hit_at_1 = MetricsCalculator.calculate_hit_at_k(
            retrieved_chunks, normalized_keywords, 1
        )
        hit_at_3 = MetricsCalculator.calculate_hit_at_k(
            retrieved_chunks, normalized_keywords, 3
        )
        hit_at_5 = MetricsCalculator.calculate_hit_at_k(
            retrieved_chunks, normalized_keywords, 5
        )
        hit_at_10 = MetricsCalculator.calculate_hit_at_k(
            retrieved_chunks, normalized_keywords, 10
        )

        keyword_recall = MetricsCalculator.calculate_keyword_recall(
            retrieved_chunks, normalized_keywords
        )
        keyword_precision = MetricsCalculator.calculate_keyword_precision(
            retrieved_chunks, normalized_keywords
        )
...
class EvaluationSummary:
    ...
    def __init__(self, query_metrics: List[QueryMetrics]):
        ...
        # Compute aggregates
        self.hit_at_1_rate = (
            sum(1 for m in query_metrics if m.hit_at_1) / len(query_metrics)
            if query_metrics
            else 0.0
        )
        self.hit_at_3_rate = (
            sum(1 for m in query_metrics if m.hit_at_3) / len(query_metrics)
            if query_metrics
            else 0.0
        )
        self.hit_at_5_rate = (
            sum(1 for m in query_metrics if m.hit_at_5) / len(query_metrics)
            if query_metrics
            else 0.0
        )
        self.hit_at_10_rate = (
            sum(1 for m in query_metrics if m.hit_at_10) / len(query_metrics)
            if query_metrics
            else 0.0
        )

        self.avg_keyword_recall = (
            sum(m.keyword_recall for m in query_metrics) / len(query_metrics)
            if query_metrics
            else 0.0
        )
        self.avg_keyword_precision = (
            sum(m.keyword_precision for m in query_metrics) / len(query_metrics)
            if query_metrics
            else 0.0
        )
        self.avg_mrr = (
