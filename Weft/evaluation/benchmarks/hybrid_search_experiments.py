...
from Weft.evaluation.core.memory_metrics import MemoryMetricsCalculator, MemoryRetrievalResult, MemoryEvaluationSummary
    ...
    for name, init_retriever in strategies.items():
        ...
        for q_idx, q in enumerate(queries):
            ...
            if name == "pure_semantic":
                ...
                raw_results = retriever.retrieve(query_text, k=20, fusion_strategy="rrf")
            elif name == "hybrid_linear_0.5":
                raw_results = retriever.retrieve(query_text, k=20, fusion_strategy="linear", alpha=0.5)
            ...
            for r in raw_results:
                mem_results.append(MemoryRetrievalResult(
                    rank=r.rank,
                    distance=r.distance,
            ...
            if metrics.candidate_recall_at_10: eval_summary.queries_with_candidate_recall_at_10 += 1
            if metrics.candidate_recall_at_20: eval_summary.queries_with_candidate_recall_at_20 += 1
            if metrics.candidate_recall_at_50: eval_summary.queries_with_candidate_recall_at_50 += 1
