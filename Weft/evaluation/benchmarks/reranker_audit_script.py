...
def evaluate_with_reranker(dataset: List[Dict[str, Any]], reranker: CrossEncoderReranker, top_n: int) -> dict:
...
def run_ablation_tests(dataset: List[Dict[str, Any]]):
    ...
    for top_n in [10, 20, 50, 100]:
        ...
        print(f"Top {top_n:<3} Reranked -> Hit@1: {agg['hit_at_1']:.3f} | MRR: {agg['mrr']:.3f} | Latency: {agg['avg_latency_ms']:.1f}ms")
...
def run_chunking_vs_coherent_test(dataset: List[Dict[str, Any]]):
    ...
    for r in retrieved_fractured[:3]:
        print(f"Rank {r.rank} | Score {-r.distance:.4f} | {r.chunk_text[:60].replace(chr(10), ' ')}")
...
def analyze_scores(dataset: List[Dict[str, Any]]):
    ...
    print("\nAfter Reranking:")
    for r in retrieved:
        print(f"New Rank: {r.rank} | CE Score: {-r.distance:.4f} | Char count: {len(r.chunk_text)}")
