...
def evaluate_all_memory_queries(queries: List[Dict[str, Any]], 
                               verbose: bool = False,
                               max_results: int = 50) -> Tuple[MemoryEvaluationSummary, List[Dict]]:
    ...
    for i, query_dict in enumerate(queries, start=1):
        ...
        print(f"[{i}/{len(queries)}] {query_text[:60]}... → expecting: {expected_phrase}")
    ...
    if query_metrics:
        summary.queries_with_hits_at_1 = sum(1 for m in query_metrics if m.memory_hit_at_1)
        summary.queries_with_hits_at_3 = sum(1 for m in query_metrics if m.memory_hit_at_3)
        summary.queries_with_hits_at_5 = sum(1 for m in query_metrics if m.memory_hit_at_5)
        summary.queries_with_hits_at_10 = sum(1 for m in query_metrics if m.memory_hit_at_10)
        
        summary.queries_with_candidate_recall_at_10 = sum(1 for m in query_metrics if m.candidate_recall_at_10)
        summary.queries_with_candidate_recall_at_20 = sum(1 for m in query_metrics if m.candidate_recall_at_20)
        summary.queries_with_candidate_recall_at_50 = sum(1 for m in query_metrics if m.candidate_recall_at_50)
        
        summary.avg_phrase_mrr = sum(m.phrase_mrr for m in query_metrics) / len(query_metrics)
...
def _print_query_details(metrics: MemoryQueryMetrics):
    ...
    if metrics.retrieved_chunks:
        ...
        for chunk in metrics.retrieved_chunks[:3]:
            status = "✓ HAS PHRASE" if MemoryMetricsCalculator.phrase_in_text(
...
def format_memory_report(summary: MemoryEvaluationSummary, verbose: bool = False) -> str:
    ...
    # Summary metrics
    lines.append("\nMEMORY HIT RATES:")
    lines.append(f"  Hit@1:  {summary.memory_hit_at_1_rate*100:5.1f}%  ({summary.queries_with_hits_at_1}/{summary.total_queries})")
    lines.append(f"  Hit@3:  {summary.memory_hit_at_3_rate*100:5.1f}%  ({summary.queries_with_hits_at_3}/{summary.total_queries})")
    lines.append(f"  Hit@5:  {summary.memory_hit_at_5_rate*100:5.1f}%  ({summary.queries_with_hits_at_5}/{summary.total_queries})")
    lines.append(f"  Hit@10: {summary.memory_hit_at_10_rate*100:5.1f}%  ({summary.queries_with_hits_at_10}/{summary.total_queries})")
    
    lines.append("\nCANDIDATE RECALL (phrase found anywhere in range):")
    lines.append(f"  Recall@10: {summary.candidate_recall_at_10_rate*100:5.1f}%  ({summary.queries_with_candidate_recall_at_10}/{summary.total_queries})")
    lines.append(f"  Recall@20: {summary.candidate_recall_at_20_rate*100:5.1f}%  ({summary.queries_with_candidate_recall_at_20}/{summary.total_queries})")
    lines.append(f"  Recall@50: {summary.candidate_recall_at_50_rate*100:5.1f}%  ({summary.queries_with_candidate_recall_at_50}/{summary.total_queries})")
    ...
    if summary.candidate_recall_at_50_rate < 0.5:
        ...
        lines.append("    → Consider: different embedding model, different chunking strategy")
    ...
    if summary.per_query_metrics:
        ...
        for m in best:
            ...
            lines.append(f"    Expected: {m.expected_phrase} | Found at rank: {m.phrase_rank} | MRR: {m.phrase_mrr:.4f}")
...
def main():
    ...
    summary, reranking_candidates = evaluate_all_memory_queries(queries, verbose=args.verbose)
