...
class VectorRetriever:
    ...
    def retrieve(
        self, query: str, k: int = 10, session: Optional[Session] = None
        ...
        try:
            # 1. Encode query
            vector_embedding = self.model.encode(
...
class RetrievalPipeline:
    ...
    def search(
        self, query: str, top_n: int = 100, final_k: int = 10
    ) -> List[RetrievalResult]:
            ...
            for rank, row in enumerate(results, start=1):
                ...
                retrieved.append(
                    RetrievalResult(
                        ...
                        distance=-float(
                            row.rank_score
                        ),  # Negative score to maintain "lower is better" sorting
                        conversation_id=row.conversation_id,
                        message_id=row.message_id,
                        chunk_order=row.chunk_order,
                        rank=rank,
...
class HybridRetriever:
    ...
    def _rrf_fusion(
        self, vec_results, lex_results, k=10, rrf_k=60
    ) -> List[RetrievalResult]:
        scores = {}
        chunks = {}

        def get_key(r):
            return f"{r.conversation_id}_{r.message_id}_{r.chunk_order}"
    ...
    def _linear_fusion(
        self, vec_results, lex_results, alpha=0.5, k=10
    ) -> List[RetrievalResult]:
        scores = {}
        chunks = {}
