import pytest
import uuid
from Weft.storage.models import Conversation, Message, MemoryType, Memory, MemoryEvidence
...
def test_message_model_creation():
    ...
    msg = Message(id=msg_id, role="user", content="Hello", conversation_id=str(uuid.uuid4()))
...
def test_memory_models_creation():
    ...
    memory = Memory(id=mem_id, type_id=type_id, value={"target": "Python"}, status="active")
    assert memory.value == {"target": "Python"}
    assert memory.status == "active"
    
    evidence_id = str(uuid.uuid4())
    evidence = MemoryEvidence(id=evidence_id, memory_id=mem_id, message_id=str(uuid.uuid4()), confidence_score=0.95)
